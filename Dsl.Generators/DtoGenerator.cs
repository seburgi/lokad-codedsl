...
public partial class {0}";
        ...
        private void WriteEquals(CodeWriter writer, Message message)
        {
            ...
            var simpleMembers = message.Members.Where(x => !x.Type.EndsWith("[]") && !x.Type.Contains("List<")).ToList();

            if (!simpleMembers.Any())
            {
                writer.WriteLine("return true;");
            }
            else
            {
                writer.WriteLine("return " + String.Join(" && ", simpleMembers.Select(x => GeneratorUtil.MemberCase(x.Name) + " == other." + GeneratorUtil.MemberCase(x.Name))) + ";");
            }
            writer.Indent--;
            writer.WriteLine("}");
            ...
            WriteParameters(simpleMembers, writer);
            writer.WriteLine(")");
            writer.WriteLine("{");
            writer.Indent++;
            if (!simpleMembers.Any())
            ...
            {
                writer.WriteLine("return " + String.Join(" && ", simpleMembers.Select(x => GeneratorUtil.MemberCase(x.Name) + " == " + GeneratorUtil.ParameterCase(x.Name))) + ";");
            }
            writer.Indent--;
            writer.WriteLine("}");
        }
...
// ReSharper disable UnusedMember.Local");
        ...
        private void WriteContext(CodeWriter writer, Context context)
        {
            foreach (var contract in context.Contracts)
            {
                ...
                if (context.Templates.ContainsKey("WriteEquals") && context.Templates["WriteEquals"] == "True")
                    WriteEquals(writer, contract);
